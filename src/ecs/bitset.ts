 ...
 *
 * ```typescript
 * const set = createBitSet(32) // Create a new BitSet with a size of 32
 * set.or(1) // Set the bit at index 1
 * set.has(1) // true
 *
 * set.has(2) // false
 * set.xor(2) // Flip the bit at index 2
 * set.has(2) // true
 * ```
 *
 * @param size The default size of the mask array (it will grow if needed)
 */
