...
import { VertexAttribute } from '@/resource/attribute'; // replace with your actual module path
...
describe('Tracked Resource', () => {
  ...
  it('should not define version property on nested properties', () => {
    expect(Reflect.ownKeys(trackedResource.z)).not.toContain(
      resourceVersionSymbol,
    );
  });
  ...
  it('should track map objects', () => {
    ...
    expect(isTracked(trackedMap.get('position')!), 'trackedMap.position').toBe(
      true,
    );
    expect(
      isTracked(trackedMap.get('position')!.data),
      'trackedMap.position.data',
    ).toBe(true);
...
describe('Tracked Resource with Delegate', () => {
  ...
  it('should not define version property on tracked resource', () => {
    expect(Reflect.ownKeys(trackedResource)).not.toContain(
      resourceVersionSymbol,
    );
  });
});
...
describe('Tracker Class', () => {
  ...
  it('should update track states for multiple resources', () => {
    ...
    trackedResource.x = 2;
    expect(tracker.getTrackStates(trackedResource, anotherResource)).toEqual([
      TrackState.STALE,
      TrackState.FRESH,
    ]);
  });
