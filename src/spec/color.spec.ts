...
describe('colorUtils', () => {
  ...
  it('color4fToHex should convert Color4f array to hex number', () => {
    const hex: number = colorUtils.color4fToHex(
      new Float32Array([1, 0.3411764705882353, 0.2, 1]),
    );
    expect(hex).toBe(0xff5733);
  });

  it('srgbToLinear should convert sRGB Color4f to linear Color4f', () => {
    const linearColor: Color4f = colorUtils.srgbToLinear(
  ...
  it('fromLinear should convert linear Color4f to sRGB Color4f', () => {
    const srgbColor: Color4f = colorUtils.fromLinear(
      new Float32Array([
        0.21404114048223255, 0.21404114048223255, 0.21404114048223255, 1,
      ]),
    );
