...
export class RendererList {
  ...
  update() {
    ...
    this._renderers.sort((a, b) => {
      ...
      const aDistance = vec3.distance(cameraPosition, aPosition);
      const bDistance = vec3.distance(cameraPosition, bPosition);

      return bMaterial.blendMode === MaterialBlendMode.BLEND
        ? bDistance - aDistance
        : aDistance - bDistance;
    });
