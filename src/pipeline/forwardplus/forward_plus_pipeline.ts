...
export class ForwardPlusPipeline extends ScriptablePipeline {
    ...
    renderMeshRenderer(context: RenderContext, renderer: MeshRendererComponent) {
        ...
        if (mesh.indices) {
            ...
            const format: GPUIndexFormat = mesh.indices.data instanceof Uint16Array ? 'uint16' : 'uint32'
            passEncoder.setIndexBuffer(indexBuffer.buffer, format, indexBuffer.offset, indexBuffer.size)
